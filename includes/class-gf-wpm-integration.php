...
class Gf_Wpm_Integration {
	...
	private function define_public_hooks() {
		...
		//$this->loader->add_action( 'wp_head', $plugin_public, 'wp_head_func' );
		...
		*/
		...
		/**
		 * Register Nav Menus for Workflow
		 */

		$this->loader->add_action( 'after_setup_theme', $plugin_public, 'workflow_register_nav_menu', 10 );

		/** Filter Status Column */

		$this->loader->add_filter( 'gravityflow_columns_status_table', $plugin_public, 'gravityflow_columns_status_func', 100, 3 );

		/** 
		 * Set Pm Comments
		 */

		$this->loader->add_action( 'gform_after_submission_18',  $plugin_public, 'set_pm_comments', 10, 2 );

		/**
		 * Remove & Add Action for New Comment Noticiation
		 */

		$this->loader->add_action( 'init', $plugin_public, 'init_func', 20 );

		$this->loader->add_action('pm_after_new_comment_notification',  $plugin_public, 'email_notification_trigger', 10, 2 );
	
		/**
		 * 	$args = apply_filters( 'gravityflow_status_args', $args );
		 */

		$this->loader->add_filter( 'gravityflow_status_args', $plugin_public, 'gravityflow_status_args_func' );
